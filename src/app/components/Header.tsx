        ...
        <header
            ...
            <nav className={"flex justify-center items-center gap-4 text-lg"}>
                {links.slice(1, links.length).map((link) => (
                    <Link key={link.label} href={link.href} className={`p-2 rounded-md shadow-2xl ${pathname.includes(link.href) ? "text-xl text-palette-black dark:bg-palette-black dark:text-white bg-white" : "bg-palette-black dark:bg-white"}`}>{link.label}</Link>
                ))}
                <ThemeSwitcher/>
            </nav>
        </header>
